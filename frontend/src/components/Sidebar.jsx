...
import { Plus, Sparkles, MessageSquare, Compass, ChevronLeft, ChevronRight, History } from 'lucide-react';
          ...
          <div className="overflow-hidden">
            <div className="flex items-center gap-1.5">
              ...
              <span className="px-1.5 py-0.5 rounded bg-amber-100 text-amber-800 text-[10px] font-bold border border-amber-300 flex items-center gap-0.5 shrink-0">
                <Sparkles className="w-2.5 h-2.5" /> Task 9
              </span>
            </div>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-4 custom-scrollbar">
        ...
        {!isCollapsed && (
          <div className="space-y-2 pt-2 border-t border-slate-200/80">
            <h3 className="text-[11px] font-bold text-slate-500 uppercase tracking-wider px-1 flex items-center gap-1">
            ...
            <div className="space-y-1">
              {conversations.map((conv) => (
                <button
                  ...
                  className={`w-full p-2 rounded-xl text-left transition-colors flex items-center gap-2 text-xs ${
                    activeConvId === conv.id
                      ? 'bg-teal-100/80 text-teal-900 font-bold border border-teal-300'
